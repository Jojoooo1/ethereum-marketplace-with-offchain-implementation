...
module.exports = function(callback) {
  ...
  EcommerceStore.deployed().then(function(f) {
    f.addStore("store1", "clothes", "test", "description", { from: web3.eth.accounts[8], gas: 200000 }).then(function(f) {
      console.log(f);
    });
  });
  EcommerceStore.deployed().then(function(f) {
    f.addStore("store2", "clothes", "test", "description", { from: web3.eth.accounts[9], gas: 200000 }).then(function(f) {
      console.log(f);
    });
  });

  EcommerceStore.deployed().then(function(f) {
    f.approveStore(web3.eth.accounts[8], { from: web3.eth.accounts[0] }).then(function(f) {
      console.log(f);
    });
  });
  EcommerceStore.deployed().then(function(f) {
  ...
  EcommerceStore.deployed().then(function(f) {
    f.addProductToStore("name: product8", "category: clothes", 3, "imageLink: test", "descriptionLink: test", web3.toWei(2), {
      from: web3.eth.accounts[9],
      gas: 2000000
    }).then(function(f) {
      console.log(f);
